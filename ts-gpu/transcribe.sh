#!/bin/bash
# transcription stream transcription and diarization script
...
for sub_dir in "${sub_dirs[@]}"; do
    ...
    for incoming_dir in "$type_dir"*/ ; do
        ...
        for ext in "${audio_extensions[@]}"; do
            ...
            for audio_file in "$incoming_dir"*."$ext"; do
                ...
                if [ "$sub_dir" == "diarize" ]; then
                    ...
                    python3 diarize_parallel.py --batch-size 16 --whisper-model $DIARIZATION_MODEL -a "$audio_file"
        ...
        then
            ...
            python3 /root/scripts/notify.py --path "$transcribed_dir$row_id/"
        fi
